...
export default function PersonalLoanEMIClient() {
  // Personal loan specific defaults
  ...
  useEffect(() => {
    ...
    setIsClient(true);
    try {
      const saved = localStorage.getItem('personal_loan_emi_history');
      if (saved) {
  ...
  const handleSaveCalculation = () => {
    ...
    if (typeof window !== 'undefined' && window.gtag) {
      window.gtag('event', 'personal_loan_emi_saved', {
        loan_amount: amount,
        interest_rate: rate,
        tenure_years: tenure,
      });
  ...
  const handleClearAll = () => {
    ...
    try {
      localStorage.removeItem('personal_loan_emi_history');
    } catch (error) {
  ...
  const handleShare = () => {
    ...
    if (typeof window !== 'undefined' && window.gtag) {
      window.gtag('event', 'personal_loan_emi_shared', {
        method: 'whatsapp',
      });
    }
  };
        ...
        <Card className="border-purple-200 bg-linear-to-br from-purple-50 to-white">
          <CardHeader>
            ...
            <p className="text-sm text-slate-600 mt-2">
              Prepaying personal loans saves maximum interest due to high rates
            </p>
          </CardHeader>
